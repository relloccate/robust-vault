<template>
    <div class="release">
        ...
        <div class="assets">
            <div class="asset" v-for="asset in getFilteredAssets" :key="asset.id">
                <a :href="asset.browser_download_url" @click="openLink">{{ asset.browser_download_url }}</a>
            </div>
        </div>
    </div>
</template>
<script>
import { openLink } from 'core/Misc';
...
export default {
    ...
    setup() {
        const { getUpdatableReleases } = useUpdatesStore;
...
.release {
    ...
    & .assets {
        ...
        & .asset {
            border-bottom: 1px dashed color-mod(var(--grey-color) a(5%));
